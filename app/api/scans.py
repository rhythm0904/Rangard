...
async def upload_and_scan(
    ...
    logger.info(
        f"[Scan] Starting scan: {file.filename} "
        f"({len(file_data)} bytes) for user {current_user.email}"
    )

    # ── 2. Create a DB record immediately (status = scanning) ──────────────
    # This lets the frontend poll for results even before the scan completes
    scan = FileScan(
        user_id=current_user.id,
        original_filename=file.filename or "unknown",
        file_size_bytes=len(file_data),
        mime_type=file.content_type,
        sha256_hash="pending",   # filled in after ML scan
    ...
    try:
        ...
        if result.threat_level in QUARANTINE_LEVELS:
            ...
            quar_record = QuarantineRecord(
                ...
                reason=f"AI detection: {result.threat_level} threat "
        ...
        return UploadResponse(
            scan_id=scan_id,
            status="complete",
            message=_result_message(result.threat_level, is_quarantined),
            threat_level=result.threat_level,
            confidence=result.confidence,
...
async def _send_alert_async(email_svc, **kwargs):
    ...
    try:
        loop = asyncio.get_event_loop()
        await loop.run_in_executor(None, lambda: email_svc.send_threat_alert(**kwargs))
    except Exception as e:
        logger.error(f"[Email] Background send failed: {e}")
