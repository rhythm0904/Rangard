...
<div style={{padding:'40px 24px 48px', background:'linear-gradient(180deg,rgba(8,13,26,0.85),#04060f)'}}>
  <div style={{textAlign:'center',fontSize:10,fontWeight:700,color:'#3d4f6b',letterSpacing:'.2em',textTransform:'uppercase',marginBottom:28}}>
    Trusted by security teams
  </div>
  <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:14,maxWidth:960,margin:'0 auto'}}>
    {[
      { name:'Sarah Mitchell', role:'CTO, FinanceCore Ltd', avatar:'SM', text:'RANGARD caught a Ryuk variant in our document pipeline that our legacy AV completely missed. The blockchain proof was invaluable for our compliance audit.', color:'#00e5ff' },
      { name:'James Okafor',   role:'IT Security Lead, MedSync', avatar:'JO', text:'We deployed RANGARD across our hospital network. The ThreatPulse alerts mean our team responds in minutes, not hours. Genuinely impressive detection accuracy.', color:'#a78bfa' },
      { name:'Priya Sharma',   role:'DevOps Engineer, CloudStack', avatar:'PS', text:'The real-time scanning API integrates perfectly into our CI/CD pipeline. Zero false positives in 3 months of production use. This is the future of file security.', color:'#f0047f' },
    ].map((t,i)=>(
      <div key={i} style={{background:'linear-gradient(135deg,#080d1a,#04060f)',border:`1px solid ${t.color}22`,borderRadius:14,padding:22,transition:'all .3s',cursor:'default'}}
        onMouseEnter={e=>{e.currentTarget.style.borderColor=t.color+'55';e.currentTarget.style.boxShadow=`0 8px 32px ${t.color}15`}}
        onMouseLeave={e=>{e.currentTarget.style.borderColor=t.color+'22';e.currentTarget.style.boxShadow='none'}}>
        <div style={{fontSize:13,color:'#8896b0',lineHeight:1.7,marginBottom:18,fontStyle:'italic'}}>
          "{t.text}"
        </div>
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          <div style={{width:38,height:38,borderRadius:'50%',background:`linear-gradient(135deg,${t.color}33,${t.color}11)`,border:`1px solid ${t.color}44`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,fontWeight:700,color:t.color,flexShrink:0}}>
            {t.avatar}
          </div>
          <div>
  ...
  <div style={{display:'flex',justifyContent:'center',gap:24,marginTop:36,flexWrap:'wrap'}}>
    {[
      ...
      <div key={i} style={{display:'flex',alignItems:'center',gap:7,padding:'8px 16px',background:'rgba(255,255,255,0.03)',border:'1px solid #1e2d52',borderRadius:20,fontSize:11,color:'#8896b0'}}>
